...
async function main() {
  ...
  biocideQuestions.forEach((q: any) => {
    console.log(`Order ${q.Order}: ${(q.Name || '').substring(0, 60)}`)
    console.log(`  Response type: ${q['Response type']}`)
    console.log(`  Dependent no show: ${q['Dependent no show']}`)
    console.log(`  Show when parent: ${q['Show when parent selected']}`)
    console.log('')
  })
}

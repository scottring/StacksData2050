...
const logger = createLogger('FixAllFKsComplete');
...
const sheetCache = new Map<string, string>();  // NEW: Sheet by name
...
async function preloadCaches() {
  ...
  for (const s of sheets || []) {
    if (s.name) {
      sheetCache.set(s.name.toLowerCase().trim(), s.id);
    }
  }
  logger.info(`  Sheets: ${sheetCache.size}`);
  ...
  // Questions by name (correct column name!)
  ...
  for (const q of questions || []) {
    if (q.name) {
      questionCache.set(q.name.toLowerCase().trim(), q.id);
    }
  }
  ...
  logger.info('  Loading list table rows...');
  let allRows: any[] = [];
  let offset = 0;
  let hasMore = true;
...
async function main() {
  logger.info('Starting COMPLETE FK fix for all answers...');
  ...
  for (const answer of answers) {
    ...
    // Sheet by name
    const sheetName = normalize(answer.Sheet);
    if (sheetName) {
      const sheetId = sheetCache.get(sheetName);
      if (sheetId) {
  ...
  logger.info(`  Sheet: ${notFoundStats.sheet}`);
  ...
  logger.info('');
  logger.success(`Import complete! Updated: ${updated}, Errors: ${errors}`);
}

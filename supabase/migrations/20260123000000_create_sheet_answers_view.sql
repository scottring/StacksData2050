-- Migration: Create sheet_answers_display view
-- This view provides pre-deduplicated, pre-joined answer data for the web app
-- reducing 13 queries + client-side processing to a single query.
--
-- NOTE: The import script now handles deduplication by keeping only answers
-- from the latest sheet version. This view provides additional safety
-- by deduplicating any remaining duplicates by modified_at.

-- Drop existing view if it exists
DROP VIEW IF EXISTS sheet_answers_display;

-- Create the view with deduplication logic
...
COMMENT ON VIEW sheet_answers_display IS 'Pre-deduplicated answer view for web app. Deduplicates answers by keeping the most recent per (sheet, question) for single answers, and per (sheet, question, row, column) for list table answers.';

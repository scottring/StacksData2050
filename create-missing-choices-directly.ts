...
console.log('=== CREATING MISSING CHOICES FOR PROBLEMATIC QUESTIONS ===\n')

// List of question IDs from the remap output that are missing choices
...
// Remove duplicates
const uniqueQuestionIds = [...new Set(questionIdsNeedingChoices)]
console.log(`Processing ${uniqueQuestionIds.length} unique questions\n`)
...
for (const questionId of uniqueQuestionIds) {
  ...
  const { data: question } = await supabase
    .from('questions')
    .select('id, name, bubble_id, order_number, parent_section_id')
    .eq('id', questionId)
    .maybeSingle()

  if (!question) {
    console.log(`  ⚠️  Question ${questionId} not found in database`)
    questionsFailed++
    continue
  }

  if (!question.bubble_id) {
    console.log(`  ⚠️  Question ${questionId} has no bubble_id`)
    questionsFailed++
    continue
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.log(`  ❌ HTTP ${response.status} for question Q${question.order_number}: ${question.name?.substring(0, 40)}`)
      questionsFailed++
      continue
    }
    ...
    if (bubbleChoices.length === 0) {
      console.log(`  ⚠️  No choices in Bubble for Q${question.order_number}: ${question.name?.substring(0, 40)}`)
      questionsFailed++
      continue
    }
    ...
    for (const bubbleChoice of bubbleChoices) {
      ...
      if (error && !error.message.includes('duplicate')) {
        console.log(`  ❌ Error creating "${content}" for Q${question.order_number}: ${error.message}`)
      } else if (!error) {
        choicesCreated++
    ...
    if (createdForThisQuestion > 0) {
      questionsProcessed++
      console.log(`  ✓ [${questionsProcessed}/${uniqueQuestionIds.length}] Created ${createdForThisQuestion} choices for Q${question.order_number}: ${question.name?.substring(0, 50)}`)
    }
...
console.log(`\n=== IMPORT COMPLETE ===`)
console.log(`Questions processed successfully: ${questionsProcessed}`)
console.log(`Choices created: ${choicesCreated}`)
console.log(`Questions failed: ${questionsFailed}`)
console.log(`\nNext: Run remap script again to fix the remaining orphaned answers`)
